...
enum { REDO_ROOT, REDO_ALLOC_BLOCK };
...
static int log_root_change (log_s *log, buf_s *buf)
{
	super_s		*super = buf->b_data;
	logrec_s	logrec;
	int		rc;
FN;
printf("ROOT=%llx\n", buf->b_blknum);
	logrec.lr_user[0] = super->sp_root;
	logrec.lr_user[1] = 0;
	logrec.lr_user[2] = 0;
	logrec.lr_user[3] = 0;
	logrec.lr_slot = STRING_SLOT;
	logrec.lr_fn   = REDO_ROOT;
	logrec.lr_len  = 0;
	bind_chkpt(log, buf);
	rc = rec_log(log, &logrec, NULL);
	if (rc) return rc;
...
FN;
	...
	log_root_change(tree->t_log, buf);
	bdirty(buf);
	bput(buf);
	return 0;
}

static int redo_root (log_s *log, logrec_s *logrec)
{
	super_s	*super;
	buf_s	*buf;
FN;
	...
	if (!buf) {
		eprintf("redo_root: no super block");
		return qERR_NOT_FOUND;
	}
	super = buf->b_data;
	...
	super->sp_lsn = logrec->lr_lsn;
	super->sp_root = logrec->lr_user[0];
printf("REDO ROOT=%llx\n", super->sp_root);
	bdirty(buf);
	bput(buf);
	return 0;
}

static int log_alloc_block (log_s *log, buf_s *buf)
{
	super_s		*super = buf->b_data;
	logrec_s	logrec;
	int		rc;
FN;
	logrec.lr_user[0] = super->sp_next;
	logrec.lr_user[1] = 0;
	logrec.lr_user[2] = 0;
	logrec.lr_user[3] = 0;
	logrec.lr_slot = STRING_SLOT;
	logrec.lr_fn   = REDO_ALLOC_BLOCK;
	logrec.lr_len  = 0;
	bind_chkpt(log, buf);
	rc = rec_log(log, &logrec, NULL);
	if (rc) return rc;
...
FN;
	...
	log_alloc_block(bgetlog(dev), buf);
	bdirty(buf);
	bput(buf);
