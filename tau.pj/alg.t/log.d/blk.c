...
static void nil_flush (agent_s *agent)
{
	buf_s	*buf = container(agent, buf_s, b_agent);
...
static void flush (agent_s *agent)
{
	buf_s	*buf = container(agent, buf_s, b_agent);
	dev_s	*dev;
	int	rc;
...
void binit (int num_bufs)
{
	...
	for (i = 0; i < num_bufs; i++) {
		...
		init_agent( &Buf[i].b_agent, flush);
	}
	Num_bufs = num_bufs;
}
...
{
printf("PUT %llx %llx\n", buf->b_blknum, *((u64 *)buf->b_data));
if (!buf->b_dirty) assert_no_bonds( &buf->b_agent);
	assert(buf->b_use > 0);
	if (!buf->b_dev->d_lazy) {
		bflush(buf);
	}
...
printf("LOOKUP %llx %llx\n", buf->b_blknum, *((u64 *)buf->b_data));
	...
	buf = victim();
assert_no_bonds( &buf->b_agent);
	init_agent( &buf->b_agent, flush);
	buf->b_dev = dev;
	buf->b_blknum = blknum;
	add(buf);
...
static void do_nil_flush (void)
{
	unint	i;

	for (i = 0; i < Num_bufs; i++) {
		Buf[i].b_agent.ag_flush = nil_flush;
	}
	for (i = 0; i < Num_bufs; i++) {
		bflush( &Buf[i]);
	}
