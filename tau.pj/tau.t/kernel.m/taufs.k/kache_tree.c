...
{
	dprintk("LEAF block=%p num=%x end=%x total=%x\n",
...
static void *alloc_node (void)
{
	void	*p;
FN;
	p = (void *)__get_free_page(GFP_KERNEL | __GFP_HIGHMEM);
	if (!p) {
		eprintk("no memory");
	}
	return p;
	...
	unint	depth)
{
	...
	case BRANCH:	dump_branch(tree, (branch_s *)h, depth);
			break;
	default:	tau_prmem("Unknown block magic", h, PAGE_SIZE);
			break;
	}
}
...
{
	dprintk("Tree------------------------------------------------\n");
	dump_node(tree, (void *)root(tree), 0);
}
	...
	unint		height)
{
	...
	case BRANCH:	stat_branch(node, bs, height + 1);
			break;
	default:	eprintk("Unknown block magic %x", h->h_magic);
			break;
	}
...
void kache_stat_tree (tree_s *tree, btree_stat_s *bs)
{
	zero(*bs);
	bs->st_blk_size = PAGE_SIZE;
	stat_node((void *)root(tree), bs, 1);
}
	...
	branch_s 	*bbranch)
{
	...
	for (i = 0; i < abranch->br_num; i++) {
		...
		if (akey->k_node != bkey->k_node) {
			eprintk("key block %lu doesn't match %p!=%p",
...
static int compare_nodes (tree_s *a, tree_s *b, head_s *ahead, head_s *bhead)
{
	...
	if (ahead->h_node != bhead->h_node) {
		eprintk("Blknum doesn't match %p!=%p",
...
{
	return compare_nodes(a, b, (void *)root(a), (void *)root(b));
}
...
static bool split_leaf (tree_s *tree, branch_s *parent, leaf_s *child, s64 k, unint size)
{
	leaf_s	*sibling;
	snint	upper;
	u64	upper_key;
...
static bool split_branch (tree_s *tree, branch_s *parent, branch_s *child, s64 k)
{
	branch_s	*sibling;
	snint		upper;
	snint		midpoint;
...
int kache_insert (tree_s *tree, u64 key, void *rec, unint size)
{
	void	*node;
FN;
...
FN;
	...
	eprintk("Key not found %llx", key);
	return qERR_NOT_FOUND;
}

//XXX: join doesn't free the memory that we are finish with
...
int kache_delete (tree_s *tree, u64 key)
{
	void		*child;
	branch_s	*parent;
	snint		k;
