...
prompt("receive");
	...
	for (i = 0;; i++) {
prompt("write_data");
		msg.m_method = TST_WRITE_DATA;
		msg.dt_num_bytes = sizeof Buf;
		msg.dt_seed      = i;
		rc = call_tau(data_key, &msg);
		if (rc) failure("call", rc);

		fill_data(Buf, sizeof Buf, msg.dt_seed);

		rc = write_data_tau(msg.q.q_passed_key, sizeof Buf, Buf, 0);
		if (rc) failure("write_data", rc);

//		printf("%d.\n", i);
//		prmem("Buf", Buf, sizeof Buf);

		rc = destroy_key_tau(msg.q.q_passed_key);
		if (rc) failure("destroy_key", rc);
	}
}
