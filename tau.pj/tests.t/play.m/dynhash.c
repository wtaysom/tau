...
typedef enum node_t { NODE, FINAL } node_t;
typedef struct node_s	node_s;
struct node_s {
	node_t	type;
	node_s	*node[NUM_NODES];
};
...
x_s *new_x (unint x)
{
	...
	xp = malloc(sizeof(x_s));
	memset(xp ,0, sizeof(x_s));
	xp->x = x;
	return xp;
}

/* Bret Mulvey http://bretm.home.comcast.net/~bretm/hash/Hash.cs */
u32 Sbox[256] = {
...
u32 hash32shiftmult (u32 key)
{
	u32 c2=0x27d4eb2d; // a prime or an odd constant
	key = (key ^ 61) ^ (key >> 16);
	key = key + (key << 3);
	key = key ^ (key >> 4);
	key = key * c2;
...
//const unint Prime = 402653189;//805306457;//29;//1610612741;
//const unint Prime = 805306457;
const unint Prime = 1610612741;
u32 hash_u32_32 (u32 x);
unint hash (unint x)
{
	//return hash_u32_32(x);
	return crc64( (u8 *)&x, sizeof(x));
	//return crc32( (u8 *)&x, sizeof(x));
	//return Prime * x;
	//return x % Prime;
	//return x * 2654435761U;	/* Knuth - who else?*/
	//return ComputeHash(x);
	//return hash64shift(x);
	//return hash32shiftmult(x);
	//return robert_jenkins(x);
}

void dhash (node_s *node, unint x)
{
	x_s	*xp;
	unint	h;

	xp = new_x(x);
	h = hash(x) >> (32-NODE_SHIFT);
	h &= NODE_MASK;
	xp->next = (x_s *)node->node[h];
	node->node[h] = (node_s *)xp;
}

void pr_hash (void *node)
{
	x_s	*xp = node;
