 ...
 *	WRITE	write version of reader/writer lock
 *	READ	read version of reader/writer lock
...
#elif WRITE || READ
	#define INIT_LOCK	__RAW_RW_LOCK_UNLOCKED
#elif TSPIN
	#define INIT_LOCK	0
#else
	#define INIT_LOCK	0
...
#elif WRITE
	__raw_write_lock( &lock->lock);
#elif READ
	__raw_read_lock( &lock->lock);
#elif TSPIN
...
#elif WRITE
	__raw_write_unlock( &lock->lock);
#elif READ
	__raw_read_unlock( &lock->lock);
#elif TSPIN
...
void *rgb (void *arg)
{
	...
	rgb_unlock(a->name, &Red);
	rdtscll(finish);
	printf("%s %g cycles per lock-unlock pair\n", a->name,
...
int main (int argc, char *argv[])
{
	...
	printf("raw spinlock\n");
#elif WRITE
	printf("raw write lock\n");
#elif READ
	printf("raw read lock\n");
#elif TSPIN
